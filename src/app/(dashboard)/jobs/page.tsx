...
import { RefreshCw, Search, MapPin, Calendar, User, Loader2, ChevronRight } from "lucide-react";
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSyncResult(`⚠ Sync note: ${data.error || "Check Cloud Function logs"}`);
      }
    } catch {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-4 animate-fade-in">
        ...
        {syncResult && (
          <div className={`text-sm px-3 py-2 rounded-md border ${syncResult.startsWith("✓") ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-yellow-500/10 border-yellow-500/20 text-yellow-400"}`}>
            {syncResult}
          </div>
        )}
            ...
            <CardContent className="p-0">
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-muted-foreground">
                      ...
                      <th className="text-left p-4 font-medium">Tech</th>
                  ...
                  <tbody>
                    {filteredJobs.map((job, i) => {
                      ...
                      return (
                        <tr key={job.id} className={`border-b border-border/50 hover:bg-accent/30 transition-colors cursor-pointer ${i % 2 === 0 ? "" : "bg-accent/10"}`}>
                          ...
                          <td className="p-4 text-muted-foreground">{job.serviceType}</td>
                          <td className="p-4 text-muted-foreground">
                            <div className="flex items-center gap-1.5">
                ...
                {filteredJobs.length === 0 && (
                  <div className="text-center text-muted-foreground py-12">
                    No jobs found. Try adjusting filters or syncing from FieldRoutes.
                  </div>
                )}
              </div>
              ...
              <div className="lg:hidden divide-y divide-border/50">
                {filteredJobs.map(job => {
                  const sc = statusConfig[job.status] || statusConfig.pending;
                  return (
                    <div key={job.id} className="p-4 hover:bg-accent/30 active:bg-accent/50 transition-colors">
